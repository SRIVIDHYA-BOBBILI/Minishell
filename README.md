...
1. Shell displays a prompt and waits for user input
2. Input is parsed into command and arguments
3. A child process is created using `fork()`
4. The command is executed via `exec()`
5. Parent process waits (or runs in background)
6. Loop continues until exit command is issued
...
* Basic parsing (no complex scripting support)
